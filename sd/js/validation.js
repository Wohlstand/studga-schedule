...
function validateField(e) 
{
	...
	switch (p.toLowerCase()) 
	{
		...
		case "email" : 
			...
			break;
		case "mailruagent" : 
			re = /^.+@+(mail.ru|bk.ru|inbox.ru|list.ru)$/;
			break;
		case "alnum" :
		case "alphanumeric" :
			re = /^[\w]*$/i;
...
function validateForm(f) 
{
	...
	if (errors > 0) 
	{
		...
		{
			err.className = "error";
			err.innerHTML = "<center>оНФЮКСИЯРЮ, ХЯОПЮБРЕ ГЮОХЯХ Б БШДЕКЕММШУ ОНКЪУ.</center>";
		}
		else
		{
			alert("Одно или несколько полей являются неполными или ошибочного содержания. Пожалуйста, исправьте ошибки в подсвеченных полях.");
		}
