...
if($act == "GetFlows")
{
    ...
    {
	    die('{"errormsg":"Missing facultet ID!"}');
    }
	...
	while(($flow_a = mysqlold_fetch_array($query)))
	{
		...
		*/
		...
		$fac["flow_name"] = $flow_a['gr_name'].grFlowNumber($flow_a['gr_year-start'], $flow_a['gr_year-end'], $days[0], $days[1] ).
		                    " ".$flow_a['gr_year-start']."-".$flow_a['gr_year-end'].
		                    (($old_schedule==1)?((!$IsEmtyFlow)?" *old*":" *Empty*"):"");
        $fac["flow_id"] = $flow_a['id_flow'];

		if(!empty($_COOKIE["flow"]) && intval($_COOKIE['flow']) == intval($flow_a['id_flow']))
		{
			$fac["selected"] = true;
	    }

		array_push($fac_opts, $fac);

		$i++;
	}

	$fac_info["list"] = $fac_opts;
	$fac_info["errormsg"] = "success";

	echo json_encode($fac_info);
...
else if($act == "GetSubGroups")
{
	...
	for($i=1; $i<=$flow['group_q']; $i++)
	{
		...
		{
			echo "document.getElementById('groups').options[".$counter."] = new Option(\"".RuCounter($i)."\", \"".$i."\");\n";
			if(!empty($_COOKIE["grp"]) && intval($_COOKIE['grp']) == $i)
			{
				echo "document.getElementById('groups').selectedIndex = ".intval($counter).";\n";
			}
		 	$counter++;
